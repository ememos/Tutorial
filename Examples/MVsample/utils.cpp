...
extern int roi_num;
extern struct rusage monitoring_beg[MAX_THREADS][MAX_ROI];
extern struct rusage monitoring_end[MAX_THREADS][MAX_ROI];
#endif
...
#ifdef MONITORING
void dmv(double *dmatrix_R, double *dmatrix_I, double *xR, double *xI, double *yR, double *yI, int DIM, int roi_num)
#else
...
/*
#ifdef MONITORING
		getrusage(RUSAGE_THREAD, &monitoring_beg[omp_get_thread_num()][roi_num]);
#endif
*/
...
/*
#ifdef MONITORING
		getrusage(RUSAGE_THREAD, &monitoring_end[omp_get_thread_num()][roi_num]);
#endif
*/

#if 0
		if (i == 0) {
			printf("### dmv thread[%d/%d], mysize[%d]\n", omp_get_thread_num(), omp_get_num_threads(), mysize);
		} else if (i == 150) {
			printf("### dmv thread[%d/%d], mysize[%d]\n", omp_get_thread_num(), omp_get_num_threads(), mysize);
		}
#endif
...
#ifdef MONITORING
void spmv(CSRComplex *A, double *xR, double *xI, double *yR, double *yI, int roi_num)
#else
...
#if 0
		if (i == 0) {
			printf("##### spmv thread[%d/%d], mysize[%d]\n", omp_get_thread_num(), omp_get_num_threads(), mysize);
		} else if (i == 500) {
			printf("##### spmv thread[%d/%d], mysize[%d]\n", omp_get_thread_num(), omp_get_num_threads(), mysize);
		}
#endif
